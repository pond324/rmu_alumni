...
const ImportPersonelData = ({fetchData}) => {
  ...
  const onDrop = useCallback(async (acceptedFiles) => {
    ...
    const extra = header.filter((col) => !PERSONEL_HEADER_COLUMN.includes(col));
  ...
  const importToDb = async () => {
    const { isConfirmed } = await alerts.confirmDialog(
      "ยืนยันนำเข้าข้อมูล",
      `คุณต้องการนำเข้าข้อมูลบุคลากร จำนวน ${rows.length} รายการหรือไม่`,
      "นำเข้าข้อมูล",
    );
    ...
    try {
      ...
      formData.append(
        "data",
        JSON.stringify(
          json.map(({ university_position, ...rest }) => ({
            ...rest,
            univercity_position: university_position,
          })),
        ),
      );
      ...
      const res = await axios.post(
        apiConfig.rmuAPI + "/president/import-personel-confirm",
        formData,
        {
  ...
  return (
    <>
      ...
      <button
        ...
        className="p-2 px-3.5 bg-white text-sm rounded-lg border col-span-2 lg:col-span-1 hover:bg-blue-500 hover:text-white border-gray-300 shadow-md flex items-center justify-center gap-2 cursor-pointer"
      >
        <Download size={18} />
        <p>นำเข้าข้อมูลบุคคลากร</p>
      </button>
      ...
      <LoadingWithProgess
        ...
        remark={"ระบบจะไม่บันทึกข้อมูลรหัสบุคลากรที่มีอยู่แล้ว"}
        percent={progress}
      />
    </>
  );
};
export default ImportPersonelData;
