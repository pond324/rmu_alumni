...
const ViewProfessorData = () => {
  const [showModal, setShowModal] = useState(false);
  const [ProfessorExit, setProfessorExit] = useState([]);

  useEffect(() => {
    const loadExcel = async () => {
      const response = await fetch("/files/professor_ex.xlsx");
              ...
              <table className="w-full">
                <thead>
                  <tr className="border-b border-gray-300 bg-blue-50 sticky top-0">
                    {ProfessorExit[0] &&
                      Object.keys(ProfessorExit[0]).map((key) => (
                        <th
                ...
                <tbody>
                  {ProfessorExit.map((row, index) => (
                    <tr className="border-b border-gray-300" key={index}>
                      {Object.values(row).map((value, i) => (
                        <td
                          className={`text-sm p-2.5 pb-3 ${index <= ProfessorExit.length - 1 && "border-r border-gray-300"}`}
                          key={i}
                        >
                          {value}
                        </td>
...
export default ViewProfessorData;
