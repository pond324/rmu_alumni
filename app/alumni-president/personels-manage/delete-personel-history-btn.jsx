...
const DeleteProfessorFormImportHistoryBtn = ({ importData, fetchData }) => {
  ...
  const handleDelete = async () => {
    ...
    const { isConfirmed } = await alerts.confirmDialog(
      "ยืนยันลบข้อมูลบุคลากร",
      "ข้อมูลบุคลากรจากการนำเข้าครั้งนี้จะถูกลบทั้งหมดและการกระทำนี้จะไม่สามารถย้อนกลับได้!",
      "ยืนยันลบ",
    );
    ...
    try {
      const res = await axios.delete(
        apiConfig.rmuAPI + `/president/delete-import-personel/${importData?.id}`,
        {
      ...
      if (res.status === 200) {
        alerts.success("ลบข้อมูลบุคลากรจากการนำเข้าครั้งนี้แล้ว!");
        setShowModal(false);
        fetchData();
      }
    } catch (error) {
        ...
        <div className="w-full lg:w-1/3 bg-white z-50 p-5 rounded-lg flex flex-col">
          <div className="w-full  flex items-start justify-between">
            <div className="flex flex-col">
              <span className="flex items-center gap-2">
                ...
                <p className="font-semibold text-[1rem]">ลบข้อมูลบุคลากร</p>
              </span>
          ...
          <div className="mt-3.5 w-full flex flex-col pt-3.5 border-t border-gray-300">
            ...
            <p className="mt-3.5 text-sm text-red-500">
              *ข้อมูลบุคลากรจากการนำเข้าครั้งนี้จะถูกลบทั้งหมด
...
export default DeleteProfessorFormImportHistoryBtn;
